...
const generateChartData = (currency: string) => {
  ...
  if (currency === "usdc") {
    return baseData.map(item => ({ ...item, value: Math.floor(item.value * 0.815) }));
  } else if (currency === "eurc") {
    return baseData.map(item => ({ ...item, value: Math.floor(item.value * 0.021) }));
  } else if (currency === "xlm") {
    return baseData.map(item => ({ ...item, value: Math.floor(item.value * 0.01) }));
  }
  
  return baseData;
};

export default function EarningsChart({ currency = "all" }: { currency: string }) {
  const data = generateChartData(currency);
  
  const getGradientColor = (currency: string) => {
    switch (currency) {
      case "usdc": return "#3B82F6"; // Blue
      case "eurc": return "#10B981"; // Green  
      case "xlm": return "#F59E0B"; // Yellow
